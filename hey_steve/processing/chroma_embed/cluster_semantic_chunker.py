...
class ClusterSemanticChunker(BaseChunker):
    ...
    def _get_similarity_matrix(self, embedding_function, sentences):
        ...
        for i in range(0, N, BATCH_SIZE):
            ...
            if embeddings is None:
                print(
                    f"Warning: embedding_function returned None for batch {i}-{i+BATCH_SIZE}. Skipping batch.")
                continue
        ...
        if embedding_matrix is None:
            return None  # Handle the case where no embeddings were generated
    ...
    def _optimal_segmentation(self, matrix, max_cluster_size, window_size=3):
        ...
        if matrix.shape[0] == 0:  # Handle empty matrix
            return []
