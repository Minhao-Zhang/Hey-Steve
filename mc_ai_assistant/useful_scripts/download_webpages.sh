...
do
  ...
  if [ -f "downloads/$filename.html" ]; then
    filesize=$(stat -c%s "downloads/$filename.html")
  ...
  wget -q "$url" -O "downloads/$filename.html"
  echo "Downloaded $url to downloads/$filename.html"
done < "$input_file"
